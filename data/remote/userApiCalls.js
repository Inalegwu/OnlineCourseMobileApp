    ...
    try{
            switch(method){
                ...
                case "GET":
                    ...
                    res = Boolean(config) ?  await axios.get(url, config) : 
                    Boolean(dataLoad) ? 
                    await axios.get(url, { params: dataLoad }) :  
                    await axios.get(url)
                    break
                case "POST":
                    Boolean(config) ? res = await axios.post(url, dataLoad, config) : res = await axios.post(url, dataLoad)
                    break
                case "PUT":
                    Boolean(config) ? res = await axios.put(url, dataLoad, config) :  res = await axios.put(url, dataLoad)
                    break
                default:
...
module.exports =  {
    ...
    fetchTopCourses :  (courseId="") => { 
                    if(courseId)  
                        ...
                        return apiCall("https://e-limi.africa/api/top_courses")
                },
    ...
    fetchCategories :  (categoryId="") =>  {
                    if(categoryId)
                        ...
                        return apiCall("https://e-limi.africa/api/categories")
            },

    // Fetch all the courses belong to a certain category
    fetchCategorywiseCourses : (categoryId="") => {
                if(categoryId)
                    return apiCall("https://e-limi.africa/api/category_wise_course",  { category_id: categoryId})
    },

    // Fetch Course by ID
    fetchCourseById : (courseId) => {
        if(courseId){
            return apiCall("https://e-limi.africa/api/course_object_by_id", {course_id: courseId})
        }
    },
